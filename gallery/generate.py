...
"""Generate the bounded deterministic Productive Bees review gallery."""
...
def build_function() -> bytes:
    ...
    lines.extend(cases.SETUP_COMMANDS)
    for placement in cases.PLACEMENTS:
        lines.extend(
            (
...
def verify_function() -> bytes:
    ...
    lines.append(
        "tellraw @a {\"text\":\"Productive Bees review gallery checked\","
        "\"color\":\"green\"}"
    )
    return text_bytes("\n".join(lines))
...
def base_files() -> dict[Path, bytes]:
    ...
    return {
        ...
        Path("datapack/pack.mcmeta"): json_bytes(
            {
                "pack": {
                    "description": "ATM 1.2.0 Productive Bees review gallery",
                    "pack_format": 48,
                }
            }
        ),
        ...
        function_root / "load.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            "tellraw @a {\"text\":\"Productive Bees review gallery loaded; "
